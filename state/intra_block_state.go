...
// Wrapper for concurrent read safe map
type StateObjects struct {
	data sync.Map // libcommon.Address => *stateObject
}

func NewStateObjects() *StateObjects {
	return &StateObjects{
		data: sync.Map{},
	}
}

func (so *StateObjects) Get(addr libcommon.Address) *stateObject {
	if v, ok := so.data.Load(addr); ok {
		return v.(*stateObject)
	}
	return nil
}

func (so *StateObjects) GetExist(addr libcommon.Address) (*stateObject, bool) {
	v, ok := so.data.Load(addr)
	if !ok {
		return nil, false
	}
	return v.(*stateObject), true
}

func (so *StateObjects) Set(addr libcommon.Address, obj *stateObject) {
	so.data.Store(addr, obj)
}

func (so *StateObjects) Delete(addr libcommon.Address) {
	so.data.Delete(addr)
}

type ExistMap struct {
	data sync.Map // libcommon.Address => struct{}
}

func NewExistMap() *ExistMap {
	return &ExistMap{
		data: sync.Map{},
	}
}

func (em *ExistMap) Exist(addr libcommon.Address) bool {
	_, ok := em.data.Load(addr)
	return ok
}

func (em *ExistMap) Set(addr libcommon.Address) {
	em.data.Store(addr, struct{}{})
}

func (em *ExistMap) Delete(addr libcommon.Address) {
	em.data.Delete(addr)
}
...
// Changing to concurrent read friendly
type IntraBlockState struct {
	// TODO: the state reader is not thread safe
	// we could lock it or use a pool of readers
	// let me use a lock for now
	// If the prefetching phase becomes a significant bottleneck, we can consider
	// implementing a pool of readers. (referring to the resourcePool)
	stateReader     state2.StateReader
	stateReaderLock sync.Mutex

	// This map holds 'live' objects, which will get modified while processing a state transition.
	stateObjects      *StateObjects
	stateObjectsDirty *ExistMap

	nilAccounts *ExistMap // Remember non-existent account to avoid reading them again
...
func New(stateReader state2.StateReader) *IntraBlockState {
	return &IntraBlockState{
		stateReader:       stateReader,
		stateReaderLock:   sync.Mutex{},
		stateObjects:      NewStateObjects(),
...
func (sdb *IntraBlockState) Reset() {
	...
	//}
	sdb.nilAccounts = NewExistMap()
	sdb.stateObjects = NewStateObjects()
	sdb.stateObjectsDirty = NewExistMap()
...
func (sdb *IntraBlockState) HasContractCreated(addr libcommon.Address) bool {
	stateObject := sdb.getStateObject(addr)
	if stateObject == nil {
		return false
	}
...
func (sdb *IntraBlockState) AddBalance(addr libcommon.Address, amount *uint256.Int) {
	...
	_, needAccount := sdb.stateObjects.GetExist(addr)
...
func (sdb *IntraBlockState) SetCodeHash(addr libcommon.Address, codeHash libcommon.Hash) {
	stateObject := sdb.GetOrNewStateObject(addr)
	if stateObject != nil {
...
func (sdb *IntraBlockState) getStateObject(addr libcommon.Address) (stateObject *stateObject) {
	// Prefer 'live' objects.
	if obj := sdb.stateObjects.Get(addr); obj != nil {
		return obj
	}

	// Load the object from the database.
	if ok := sdb.nilAccounts.Exist(addr); ok {
...
func (sdb *IntraBlockState) setStateObject(addr libcommon.Address, object *stateObject) {
	...
	sdb.stateObjects.Set(addr, object)
}
...
func updateAccount(EIP161Enabled bool, isAura bool, stateWriter state2.StateWriter, addr libcommon.Address, stateObject *stateObject, isDirty bool) error {
...
func (sdb *IntraBlockState) CommitBlock(chainRules *chain.Rules, stateWriter state2.StateWriter) error {
	for addr, bi := range sdb.balanceInc {
		if !bi.transferred {
			sdb.getStateObject(addr)
		}
